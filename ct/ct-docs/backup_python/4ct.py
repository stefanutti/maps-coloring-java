...
def kempe_chain_color_swap(graph, starting_edge, c1, c2):
    ...
    # Helping list (not to walk back on the right path)
    #
    visited_edges = []
...
############################################
def are_edges_to_join_on_the_same_cycle(e1, e2, c1, c2):

    are_edges_to_join_on_the_same_cycle = False

    return are_edges_to_join_on_the_same_cycle
...
while is_the_end_of_the_rebuild_process is False:
    ...
    if ariadne_step[0] == 2:
        ...
        if previous_edge_color_at_v1 == previous_edge_color_at_v2:
            logger.error("Unexpected condition (for F3 faces two edges have a vertex in common, and so colors MUST be different). Mario you'd better study math a bit longer")
            exit(-1)
        ...
        if previous_edge_color_at_v1 == previous_edge_color_at_v2:
            ...
            # CASE-001: Since edges at v1 and v2 are on the same Kempe cycle (with the top edge), I can also avoid the kempe chain color switching, since in this case the chain is made of three edges
            #
            the_colored_graph.delete_edge((vertex_to_join_near_v1_on_the_face, vertex_to_join_near_v1_not_on_the_face))
            the_colored_graph.delete_edge((vertex_to_join_near_v2_on_the_face, vertex_to_join_near_v2_not_on_the_face))
            ...
            # TODO: I don't know if this check is necessary, but it is better to do it
            #
            if (vertex_to_join_near_v1_on_the_face, vertex_to_join_near_v2_on_the_face) in the_colored_graph.multiple_edges():
            ...
            #
            if are_edges_to_join_on_the_same_cycle((vertex_to_join_near_v1_on_the_face, vertex_to_join_near_v1_not_on_the_face),(vertex_to_join_near_v2_on_the_face, vertex_to_join_near_v2_not_on_the_face),previous_edge_color_at_v1, previous_edge_color_at_v2) is True:
                ...
                #
                kempe_chain_color_swap((v1, vertex_to_join_near_v1_on_the_face), previous_edge_color_at_v1, previous_edge_color_at_v2)
                ...
                #
                kempe_chain_color_swap((v1, vertex_to_join_near_v1_on_the_face), previous_edge_color_at_v1, get_the_other_colors([previous_edge_color_at_v1, edge_color_of_top_edge])[0])
