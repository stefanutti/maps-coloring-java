 ...
 *          <li>Clean the code: Verify if styles are created also when not needed (create 4 styles and use the same style for more objects)
 *          <li>Clean the code: When coloring the map, do not recreate it every time but change only the styles
 *          <li>Clean the code: Remove from F the variables: x, y, w, h and startRadius, stopRadius, ... TODO FUNCTIONALITIES:
 *          <li>Transformations:
 ...
 *          <li>Add a coloring method based on the arbitrary choice the three color for central face, ocean + another face adjoining the other two
 *          <li>Add a map searching tool: for example "find a map with 13 faces" or "find a map with with at least two F6 faces confining" or ...
 *          <li>Filter maps that have less than N faces
 *          <li>Create a pause button for the automatic color algorithm (separated thread) with the possibility to manually modify the map
