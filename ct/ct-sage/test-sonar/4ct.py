    ...
    while is_the_end_of_switch_process is False:
        ...
        if are_the_same_edge(starting_edge, edges_to_check[0]) or are_the_same_edge(starting_edge, edges_to_check[1]):
            ...
            if next_e1_color == next_color:
                ...
                logger.error("Unexpected condition (next color should exist). Mario you'd better go back on paper")
                exit(-1)
...
def is_well_colored(graph):
    ...
    while is_end_of_job is False and i_vertex < len(vertices):
        ...
        colors_around_this_vertex = []
        colors_around_this_vertex.append(edges[0][2])
        colors_around_this_vertex.append(edges[1][2])
        colors_around_this_vertex.append(edges[2][2])
...
def join_faces(f1, f2, edge_to_remove_on_f1):
    ...
    if len(f1) == 2 and len(f2) == 2:
        logger.error("Unexpected condition (f2 + f2 would generate a single edge face). Mario you'd better go back on paper")
        exit(-1)
    ...
    if len(f1_plus_f2) == 2 and (f1_plus_f2[0][0] != f1_plus_f2[1][1] or f1_plus_f2[0][1] != f1_plus_f2[1][0]):
        logger.error("Unexpected condition (f2 faces have to be: [(v1, v2), (v2, v1)]. Mario you'd better go back on paper")
        exit(-1)
...
while is_the_end_of_the_reduction_process is False:
    ...
    if is_the_edge_to_remove_found is False:
        logger.error("Unexpected condition (a suitable edge has not been found). Mario you'd better go back on paper")
    ...
    if check_regularity(g_faces) is False:
        logger.error("Unexpected condition (check_regularity is False). Mario you'd better go back on paper")
        log_faces(g_faces)
        exit(-1)
...
if len(all_vertices) != 4:
    logger.error("Unexpected condition (vertices left are not 4). Mario you'd better go back on paper")
    exit(-1)
...
while is_the_end_of_the_rebuild_process is False:
    ...
    if ariadne_step[0] == 2:
        ...
        if is_well_colored(the_colored_graph) is False:
            logger.error("Unexpected condition (Not well colored). Mario you'd better go back on paper")
            exit(-1)
        ...
        # For F3 to compute the new colors is easy (check also is it is a multiple edges)
        #
        ...
        if previous_edge_color_at_v1 == previous_edge_color_at_v2:
            logger.error("Unexpected condition (for F3 faces two edges have a vertex in common, and so colors MUST be different at this point). Mario you'd better go back on paper")
            exit(-1)
        ...
        while end_of_f5_restore is False:
            ...
            if i_loop == 3 and end_of_f5_restore is False:
                ...
                print_stats()
                logger.error("Unexpected condition (endless loop). Mario you'd better go back on paper")
                exit(-1)
    ...
    if not is_well_colored(the_colored_graph):
        logger.error("Unexpected condition (coloring is not valid). Mario you'd better go back on paper or learn to code")
        exit(-1)
...
if the_colored_graph.has_multiple_edges():
    logger.error("Unexpected condition (recreated graph has multiple edges at the end). Mario you'd better go back on paper")
    exit(-1)
...
if is_isomorphic is False:
    logger.error("Unexpected condition (recreated graph is different from the original). Mario you'd better go back on paper")
