...
def get_random_edge_on_a_kempe_loop(graph, starting_edge, c1, c2):
    ...
    # Get a random edge
    #
    random_edge_to_return = edges_of_the_loop[randint(2, len(edges_of_the_loop))]

    if logger.isEnabledFor(logging.DEBUG): logger.debug("END: get_random_edge_on_a_kempe_loop: %s", random_edge_to_return)

    return random_edge_to_return
